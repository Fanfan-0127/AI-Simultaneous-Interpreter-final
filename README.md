...
GUI 设置界面一键切换，配置持久化到 `~/.fanfan-interpreter/settings.json`。
...
竞赛/会议现场 crash = 直接淘汰。多层防护：

- WASAPI 设备占用诊断 → 中文引导用户关闭冲突程序
- 启动前全链路探测 → DNS → TCP → 配置校验，失败时中文引导
...
│  LRU 缓存 (200条)  ·  Draft + Correction 双通道      │
└──────────────────┬─────────────────────────────────┘
                   ▼
┌─ 字幕展示 ─────────────────────────────────────────┐
